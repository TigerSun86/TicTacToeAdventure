...
public class BoardElementSet {
    ...
    public BoardElementSet(final Activity activity) {
        this.screenDensity =
                activity.getResources().getDisplayMetrics().density;

        initSizeData(activity);
        initIntersections();
        initPieces();
        initLines();
    ...
    private void initSizeData (final Activity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR2) {
            ...
            // Low version android. Below level 13.
            screenWidth = (float) display.getWidth();
            screenHeight = (float) display.getHeight();
        }

        oneBoardWidth = screenWidth - GAP_HOR_TOP_AHEAD - GAP_HOR_TOP_BEHIND;
        oneBoardHeight =
                (screenHeight - GAP_VER_AHEAD - GAP_VER_BEHIND - (GAP_VER_INTERVAL * 3))
                        / MAX_LEVELS;
    ...
    private float toPx (final float dp) {
